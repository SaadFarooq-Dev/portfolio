...
export default function Home() {
  return (
    <>
      <Head>
        <title>Saad Farooq</title>
        <meta name="description" content="Saad Farooq Portfolio" />
      </Head>
      <Transition />
      <main className='flex items-center text-dark w-full min-h-screen dark:text-light'>
        <Layout className='pt-0 md:pt-16 sm:pt-8 '>
          <div className="flex items-center justify-between w-full lg:flex-col">
            <div className='w-1/2 md:w-full'>
              <Image src={profileImage} alt='SaadFarooq-Dev' className='w-full h-auto lg:hidden md:inline-block md:w-full'
                priority
                sizes='(max-width: 768px) 100vw; (max-width: 1200px) 50vw; 50vw' />
            </div>
            <div className='w-1/2 flex flex-col items-center self-center lg:w-full lg:text-center'>
              <AnimatedText text='From concept to completion - bringing your ideas to life.' className="!text-6xl !text-left xl:!text-5xl lg:!text-center lg:!text-6xl md:!text-5xl sm:!tex-3xl" />
        ...
        <div className='absolute right-8 bottom-8 inline-block w-24 md:hidden'>
          <Image src={lightBuld} alt='Saad' className='w-full h-auto' />
        </div>
      </main>
    </>
  )
